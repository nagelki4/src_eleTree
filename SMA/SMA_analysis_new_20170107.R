...
###### NDVI and MSAVI2
if(ndvi_n_msavi){
setwd("C:/Users/nagelki-4/Desktop/nagelki4/Grad School/Projects/EleTree Analysis/SMA/Landsat/Mpala/LC81680602014034-SC20160914165712/Original")

# Identify mask and band file paths (we just want 4 and 5)
band4_path <- list.files(, pattern = "band4")
band5_path <- list.files(, pattern = "band5")
maskFile_path <- list.files(, pattern = "cfmask.tif") #two files have "cfmask" in them, so just use the order of files
...
rstack2 <- rstack # it looks like calc() directly manipulates rstack, so need a new one for MSAVI2
...
MSAVI2 <- calc(rstack2, function(x) (2*x[,1] + 1 - sqrt((2*x[,1]+1)^2 - 8*(x[,1] - x[,2])))/2)
...
# Calculate Stats
## RMSE
ndvi.rmse <- sqrt( mean( (master.df$SMA.soil- master.df$p.soil)^2 , na.rm = TRUE ) )
## R-squared
y.mean <- mean(master.df$SMA.soil)
...
SE.y.line <- sum((y - y.line)^2) # what is the standard error from the 1:1 line? (Google soil cover)
soil.r.sq <- 1 - SE.y.line/SE.y.mean
correl <- cor(master.df$p.soil, master.df$SMA.soil)
covar <- cov(master.df$p.soil, master.df$SMA.soil)
# Plot
