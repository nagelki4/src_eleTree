...
#######################  AUTO PLOT  #################################################
# This automatically sets is.unconstrained and TS_Only
...
c.names <- c("hdr.name", "RMSE", "adj.r.sq", "slope", "one-slope", "y-int")
colnames(mx) <- c.names
...
for(h in 1:length(hdr.list)){
  # Get file name
  SMA.14 <- substr(hdr.list[h], 0, nchar(hdr.list[h])-4) # Take off the .hdr, which isn't needed
  total.mesma.name <- gsub(pattern = "%", "prcnt", SMA.14) # remove any % signs
  print(paste(h, "of", length(hdr.list), ":", total.mesma.name))
  
  # Skip hdr files that have already had their results plotted 
  if(skip.those.already.plotted == TRUE){
    plot.list <- list.files(plot.save.folder)
    plot.name <- paste0("Tree_SMAraw_", total.mesma.name, ".png") # have to make the name specific, otherwise other names could register
  ...
  # Trees
  # Function in src_masterfunctions.R
  hdr.eval.df <- plot1to1(x = master.df$nine.tree, y = master.df$raw.SMA.tree, xlab = "Observed % Tree Cover", ylab = "Raw Predicted % Tree Cover", 
           ...
           hdr.table = hdr.eval.df, hdr.count = h, hdr.namefor.table = SMA.14)
...
write.csv(hdr.eval.df, paste0(SMA.folder, "HDR_eval_", YMD(), ".csv"))
