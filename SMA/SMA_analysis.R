...
save.class.pixel <- F  # Save the classified Google image?
...
# Code sections
play.space <- FALSE # Generally, this should be false. It stops the play space code from running when running the entire script
test.classification <- FALSE # Should be false for same reason as the play space
...
SMA.folder <- "C:/Users/nagelki-4/Desktop/nagelki4/Grad School/Projects/EleTree Analysis/SMA/Results/20161203_ForClassProject/tif/"
SMA.14 <- "14_Laik_20161203.tif"
...
########## THIS SECTION FOR TESTING CLASSIFICATION ###########################################
if(test.classification == TRUE){ 
...
#Read in Mpala raster and use to select pixels
...
red14 <- raster(paste0(landsat.folder, "mpalared_14"))
green14 <- raster(paste0(landsat.folder, "mpalagreen_14"))
blue14 <- raster(paste0(landsat.folder, "mpalablue_14"))
stack.14 <- stack(red14, green14, blue14)
plotRGB(stack.14, scale = 255, axes = TRUE, main = "2014")

red87 <- raster(paste0(landsat.folder, "mpalared_87"))
...
red14 <- mask(red14, ground.truth.frame)
green14 <- mask(green14, ground.truth.frame)
blue14 <- mask(blue14, ground.truth.frame)
red87 <- mask(red87, ground.truth.frame)
green87 <- mask(green87, ground.truth.frame)
blue87 <- mask(blue87, ground.truth.frame)
...
for(t in 1:sample.size){
  ...
  # TREES    (This doesn't work well in areas with no grass and no shadow. Could include test for % soil cover)
  # plot(sum.im.pre < 1.3 & grvd.pre > -.1)
  trees <- sum.im.pre < 1.3 & grvd.pre > -.1
  # plot(trees, main = "Trees")
  ...
  if(save.class.pixel){
    class.3030.name <- paste0("./", google.image.folder, "/pixel_", cell.numbers[t], "_3030_class_new.png")
    # Save the image
    png(class.3030.name)
    plot(tot.img, axes=F,box = F, main = "3 = Trees, 2 = Grass, 1 = Soil")
    dev.off()
...
#   # Play area for finding thresholds for images with high and low grass cover
#   red.mix <- red
#   green.mix <- green
#   blue.mix <- blue
#   
...
#   blue.grass <- blue
#   
#   mix <- mean(red.mix[]) / mean(green.mix[]) # 1.01305
#   soil.t <- mean(red.soil[]) / mean(green.soil) # 1.060796
#   grass.t <- mean(red.grass[]) / mean(green.grass[]) # 0.8323463
#   
#   mix.area <- red.mix > green.mix
#   mix.frac <- length(mix.area[mix.area == 1]) / length(mix.area) # 0.4391536
#   
#   soil.area <- red.soil > green.soil
#   soil.frac <- length(soil.area[soil.area == 1]) / length(soil.area) # 0.7730686
#   
#   grass.area <- red.grass > green.grass
#   grass.frac <- length(grass.area[grass.area == 1]) / length(grass.area) # 0.009483507
...
# Calculate Stats
##   RMSE
tree.rmse <- sqrt( mean( (master.df$SMA.tree- master.df$p.tree)^2 , na.rm = TRUE ) )
##   R-squared
y.mean <- mean(master.df$SMA.tree)
y.line <- master.df$p.tree
y <- master.df$SMA.tree
SE.y.mean <- sum((y - y.mean)^2) # what is the standard error from the mean of y? (SMA tree cover)
SE.y.line <- sum((y - y.line)^2) # what is the standard error from the 1:1 line? (Google tree cover)
tree.r.sq <- 1 - SE.y.line/SE.y.mean # NOTE 1 (see bottom) 
# Plot
plot(master.df$p.tree*100, master.df$SMA.tree*100, main = paste0("Percent Tree Cover: Observed vs. Predicted \nRMSE = ", round(tree.rmse, 2)*100, "%  R-squared = ", round(tree.r.sq, 2)), xlab = "Observed % Tree Cover", ylab = "Predicted % Tree Cover", xlim = c(0,100), ylim = c(0,100))
abline(0,1) # Add the 1:1 line
...
# Calculate Stats
## RMSE
grass.rmse <- sqrt( mean( (master.df$SMA.grass- master.df$p.grass)^2 , na.rm = TRUE ) )
## R-squared
y.mean <- mean(master.df$SMA.grass)
y.line <- master.df$p.grass
y <- master.df$SMA.grass
SE.y.mean <- sum((y - y.mean)^2) # what is the standard error from the mean of y? (SMA grass cover)
SE.y.line <- sum((y - y.line)^2) # what is the standard error from the 1:1 line? (Google grass cover)
grass.r.sq <- 1 - SE.y.line/SE.y.mean
# Plot
plot(master.df$p.grass*100, master.df$SMA.grass*100, main = paste0("Percent Grass Cover: Observed vs. Predicted \nRMSE = ", round(grass.rmse, 2)*100, "%  R-squared = ", round(grass.r.sq, 2)), xlab = "Observed % Grass Cover", ylab = "Predicted % Grass Cover", xlim = c(0,100), ylim = c(0,100))
abline(0,1) # Add the 1:1 line
...
# Calculate Stats
## RMSE
soil.rmse <- sqrt( mean( (master.df$SMA.soil- master.df$p.soil)^2 , na.rm = TRUE ) )
## R-squared
y.mean <- mean(master.df$SMA.soil)
y.line <- master.df$p.soil
y <- master.df$SMA.soil
SE.y.mean <- sum((y - y.mean)^2) # what is the standard error from the mean of y? (SMA soil cover)
SE.y.line <- sum((y - y.line)^2) # what is the standard error from the 1:1 line? (Google soil cover)
soil.r.sq <- 1 - SE.y.line/SE.y.mean
# Plot
plot(master.df$p.soil*100, master.df$SMA.soil*100, main = paste0("Percent Soil Cover: Observed vs. Predicted \nRMSE = ", round(soil.rmse, 2)*100, "%  R-squared = ", round(soil.r.sq, 2)), xlab = "Observed % Soil Cover", ylab = "Predicted % Soil Cover", xlim = c(0,100), ylim = c(0,100))
abline(0,1) # Add the 1:1 line
...
# ######### Old GE cover classification
# # Identify trees (This doesn't work well in areas with no grass and no shadow. Could include test for % soil cover)
# # par(mar=c(1.1,0.1,1.1,0.0))
# # plot(sum.im, axes=F,box = F)
# # plot(sum.im < 1, axes=F,box = F)
# trees <- sum.im < 1.3 # This does not always work well because Google imagery is from different dates all within Mpala. Will have to stay in the same image
# # plot(trees, main = "Trees", axes=F,box = F)
# 
# # Now remove the tree pixels from the image that will go into identifying soil and grass
# # create tree mask
# tree.mask <- trees
...
# grvd <- holy.image
# 
# # Split soil and grass using GRVD
# # plot(grvd, axes=F,box = F)
# # plot(grvd > -0.08, axes=F,box = F) #-0.08 seems like a pretty good number to pull out bare ground
# soil <- grvd < 0.04
# # plot(soil, main = "Soil", axes=F,box = F)
# 
# # Grass is the opposite
# grass <- grvd >= 0.04
# # plot(grass, main = "Grass", axes=F,box = F)
# 
# # Combine into one
# # Give the different cover types different values
# grass.ad <- grass
